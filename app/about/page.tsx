    ...
    <div className="mx-auto max-w-6xl px-4 py-12 md:px-6">
      <div className="grid gap-8 lg:grid-cols-[0.8fr_1.2fr] lg:items-center">
        <div>
          ...
          <h1 className="mt-3 text-4xl font-black tracking-[-0.06em] text-[var(--text-on-dark)] md:text-5xl">Coffee that feels like a ritual.</h1>
        </div>
        <div className="rounded-[30px] border border-[#2d1b12]/10 bg-white/70 p-6 text-base leading-8 text-[#4a3429] shadow-[0_18px_45px_rgba(71,47,26,0.04)]">
      ...
      <div className="mt-12 grid gap-6 md:grid-cols-3">
        {[
          ...
          <div key={item.title} className="rounded-[28px] border border-[#2d1b12]/10 bg-white/70 p-6">
            <h2 className="text-xl font-semibold text-[var(--text-on-light)]">{item.title}</h2>
            <p className="mt-3 text-sm leading-7 text-[#4a3429]">{item.body}</p>
          </div>
        ))}
      </div>
    </div>
